"""
Authentication — Option A: 2-layer auth
  Layer 1: Streamlit Cloud "Only specific people" (Google email whitelist)
  Layer 2: Portal login form (username + bcrypt password)

Hardened with:
- Rate-limit (5 fails = 15 min lockout)
- XSS escape via safe_html()
- Full session.clear() on logout
- BKK timezone for login_time
"""
...
def show_login_page():
    ...
    st.info(
        "🔒 **Portal ปลอดภัย 2 ชั้น**:\n\n"
        "1. Google email (จำกัดเฉพาะทีม — ที่คุณผ่านมาแล้ว)\n"
        "2. Portal login (username + password ที่ Admin กำหนด)"
    )
